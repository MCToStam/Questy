    ...
    pageData.forEach((country) => {
      ...
      if (dataFHomeDynmap) {
        ...
        if (dataPaysDynmap.length > 1) {
          coordTexte = `[${dataPaysDynmap[0].x[0]} 64 ${dataPaysDynmap[0].z[0]}](https://${serveur}.earthquest.fr/?worldname=earth&mapname=flat&zoom=4&x=${dataPaysDynmap[0].x[0]}&y=64&z=${dataPaysDynmap[0].z[0]})`;
        }
      }
