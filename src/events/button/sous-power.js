...
module.exports = {
  ...
  async execute(client, interaction, config) {
    const [_, pageStr, serveur] = interaction.customId.split("-");
    ...
    const container = new ContainerBuilder()
      ...
      .addTextDisplayComponents((t) =>
        t.setContent(
          `## ${config.serveur[serveur].emoji_formate} Pays en Sous-Power`,
        ),
      )
    ...
    pageData.forEach((country) => {
      ...
      if (dataFHomeDynmap) {
        ...
        if (dataPaysDynmap.length > 1) {
          coordTexte = `[${dataPaysDynmap[0].x[0]} 64 ${dataPaysDynmap[0].z[0]}](${config.serveur[serveur].dynmap}/?worldname=earth&mapname=flat&zoom=4&x=${dataPaysDynmap[0].x[0]}&y=64&z=${dataPaysDynmap[0].z[0]})`;
        }
      }
