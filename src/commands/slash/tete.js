...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("tete")
    .setDescription("Affiche la tête d'un joueur")
  ...
  async execute(client, interaction, config) {
    ...
    const apiUrl = `https://earthquest.craftserv.fr/?user=${pseudo}&headOnly=true`;
    ...
    const container = new ContainerBuilder()
      ...
      .addTextDisplayComponents((textDisplay) =>
        textDisplay.setContent(
          `## 👩‍ Tête de ${username.replace(/_/g, "\\_")}`,
        ),
      )
      .addSeparatorComponents((separator) => separator)
      .addMediaGalleryComponents((g) =>
        g.addItems((item) => item.setURL(`attachment://tete_${pseudo}.png`)),
      )
      .addSeparatorComponents((separator) => separator)
    ...
    await interaction.reply({
      ...
      files: [{ name: `tete_${pseudo}.png`, attachment: imageBuffer }],
    });
  },
};
